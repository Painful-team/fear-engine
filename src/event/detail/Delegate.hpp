...
public:
	...
	bool operator==(const Delegate& another) const noexcept { return this->object == another.object && this->stub == another.stub; }

	bool operator!=(const Delegate& another) const noexcept { return !(*this == another); }
	...
	Return operator()() const { return (*stub)(object); }

	template <auto Function, typename = std::enable_if_t<std::is_invocable_r_v<Return, decltype(Function), Params...>>>
	static Delegate create(const char* blank = nullptr)
	{
		return {nullptr, function_stub<Function>};
	}

	template <auto Method, typename T, typename = std::enable_if_t<(std::is_invocable_r_v<Return, decltype(Method), T&, Params...>)>>
	static Delegate create(T* instance)
	{
		return {instance, method_stub<Method, T>};
	}

	template <auto Method, typename T, typename = std::enable_if_t<(std::is_invocable_r_v<Return, decltype(Method), const T&, Params...>)>>
	...
	bool isNull() const noexcept { return stub == nullptr; }
